...
class IEGenericDataset(AbstractDataset):
    def __init__(self, **params):
        ...
        self.shuffle_triplets = params.get("shuffle_triplets", False)

        if params.get("load_from_huggingface", False):
            self._load_from_huggingface(huggingface_params=params['huggingface_params'])
        elif params.get("load_transformed", False):
            self._load_transformed_data(load_transformed_params=params["load_transformed_params"])
        else:
    ...
    def _load_transformed_data(self, load_transformed_params):
        self.data = []
        path = os.path.join(load_transformed_params["data_dir"], load_transformed_params["split"] + ".jsonl")
        with jsonlines.open(path, 'r') as f:
            for line in f:
                for completion in line['model_completions']:
                    sample = line.copy()  # TODO think what to do with ID
                    sample.pop("model_completions")
                    sample.pop("prompt")
                    sample['text'] = completion
    ...
    def _process_sample(self, sample):
        for triplet in sample['triplets']:
            try:
                triplet['subject'] = literal_eval(triplet['subject'])
            except:
                pass
                #logging.warning(f"Warning: not able to do literal_eval for {triplet['subject']}")
            try:
                triplet['predicate'] = literal_eval(triplet['predicate'])
            except:
                pass
                #logging.warning(f"Warning: not able to do literal_eval for {triplet['predicate']}")
            try:
                triplet['object'] = literal_eval(triplet['object'])
            except:
                pass
                #logging.warning(f"Warning: not able to do literal_eval for {triplet['object']}")
        return sample

    def _load_from_huggingface(self, huggingface_params):
        data = load_dataset(huggingface_params["path"], huggingface_params["type"],
                            split=huggingface_params["split"])
        self.data = []
        cnt = 0
        for sample in data:
            cnt += 1
            if cnt > huggingface_params["skip_first_lines"]:
    ...
    def _load_data(self, load_dataset_params):
        ...
        # Detect whether to assign weights to the datapoints
        weighted = False
        normalized_weight = False
        if load_dataset_params["weighted"]:
            weighted = True
            normalized_weight = load_dataset_params["normalized_weight"]

        self.data = []
        for obj in tqdm(json_reader, total=num_datapoints, desc=f"Loading the data from: {path}"):
            obj = self._process_sample(obj)
            obj["text"] = obj["text"].strip()

            if not weighted:
                obj["weight"] = 1.0
            else:
                assert "weight" in obj, "Weighted dataset, but no weights found in the data!"
        ...
        # Normalize weights so that the sum of weights is equal to the number of datapoints
        if normalized_weight:
            obj_weights = [obj["weight"] for obj in self.data]
            total_weight = sum(obj_weights)
            length = len(obj_weights)
            for obj in self.data:
                obj["weight"] = obj["weight"] / total_weight * length
    ...
    def _shuffle_triplets(self, dp):
        self.random_state.shuffle(dp['triplets'])
        return dp
    ...
    def __getitem__(self, idx):
        ...
        if ('target' in dp) and ('target_ids' in dp) and (dp['target_ids'] is not None) and not self.shuffle_triplets:
            target_text, target_ids = dp['target'], dp['target_ids']
        else:
            if self.shuffle_triplets:
                dp = self._shuffle_triplets(dp)
            target_text, target_ids = self._get_linearized_target(dp, self.tokenizer, self.linearization_class)
        weight = None
        if 'weight' in dp:
            weight = dp['weight']
        ent_removed = None
        if 'entities_removed' in dp:
            ent_removed = dp['entities_removed']
        if "constrained" in dp:
            constrained = dp["constrained"]
        else:
            constrained = None

        if "unconstrained" in dp:
            unconstrained = dp["unconstrained"]
        else:
            unconstrained = None

        if constrained and unconstrained:
            text = f"<text>{dp['text']}<constrained>{constrained}<unconstrained>{unconstrained}"
        else:
            text = dp["text"]
        if self.params.get("return_unlinearized", False):
            return {"id": dp["id"], "raw_text": dp["text"], "text": text, "target": target_text, "target_ids": target_ids,
                    "weight": weight, 'triplets': dp['triplets'], 'entities': dp['entities'],
                    'relations': dp['relations'], "constrained_draft": constrained,
                    "unconstrained_draft": unconstrained, "entities_removed": ent_removed}
        return {"id": dp["id"], "raw_text": dp["text"], "text": text, "target": target_text, "target_ids": target_ids, "weight": weight,
                "constrained_draft": constrained, "unconstrained_draft": unconstrained, "entities_removed": ent_removed}
...
class IEGenericOutputDataset(AbstractOutputDataset):
    ...
    def _load_data(self):
        data = read_outputs(self.params["data_dir"], self.params["filter"], self.params["subset"])

        processed = self.params["processed"]
        ...
        if not processed:
            ...
            for dp in data:
                if dp[mode] == "None":
                    dp[mode] = ""
                dp["prediction_triplets"] = self.get_text_triples(dp[mode])
                dp["target_triplets"] = self.get_text_triples(self.transform_targets(dp))

...
class AbstractOutputDataset(AbstractDataset, ABC):
    ...
    @staticmethod
    def transform_targets(item, key="triplets"):
        triplets = []
        for triplet in item[key]:
            sub = safe_literal_eval(triplet["subject"])["surfaceform"]
            rel = safe_literal_eval(triplet["predicate"])["surfaceform"]
            obj = safe_literal_eval(triplet["object"])["surfaceform"]
            triplet_text = f"[s] {sub} [r] {rel} [o] {obj} [e]"

...
def read_outputs(outputs_dir, filter='all', subset=None): # TODO add filter to the old version of reading here
    items_dict = defaultdict(dict)
    if outputs_dir.endswith(".jsonl"):
        with jsonlines.open(outputs_dir, 'r') as fp:
            for element in fp:
                assert "id" in element
                if filter == "all":
                    items_dict[element["id"]].update(element)
                elif filter == "entities_removed" and (element["entities_removed"]) and (element["entities_removed"] != "None"):
                    items_dict[element["id"]].update(element)
                elif filter == "entities_same" and ((element["entities_removed"]) is None or (element["entities_removed"] == "None")):
                    items_dict[element["id"]].update(element)
    else:
        for filename in os.listdir(outputs_dir):
            if not filename.endswith(".jsonl.gz") or filename.endswith(".jsonl"):
                continue
            ...
            if filename.endswith(".jsonl.gz"):
                ...
                with jsonlines.open(input_file_path, 'r') as fp:
                    for element in fp:
                        assert "id" in element
                        items_dict[element["id"]].update(element)
    ...
    if subset:
        with open(subset, 'r') as f:
            subset = json.load(f)
        items_dict_subset = {}
        for idx, elem in items_dict.items():
            if idx in subset:
                items_dict_subset[idx] = elem
                
        items_dict = items_dict_subset
...
def read_results(exp_dir):
    ...
    if os.path.isfile(results_path):
        ...
        for dir in os.listdir(exp_dir):
            results_path = os.path.join(exp_dir, dir, "results.json")
            if os.path.isfile(results_path):
                flag = True
                with open(results_path, "r") as f:
                    results = json.load(f)

...
def get_linearization_class(identifier: str):
    supported_linearization_classes = [
        ...
        FullyExpandedLinearizationNoSpace,
...
class FullyExpandedLinearization(LinearizationType):
    ...
    def triplet_list_to_text(
        ...
        if len(surface_forms["{separator}"]) > 0:
        ...
        if len(linearized_triplets_text_parts) > 0:
            ...
            tokenized_text = tokenized_linearized_triplets_text_parts[0]
...
class FullyExpandedLinearizationNoSpace(FullyExpandedLinearization):
    identifier = "fully_expanded_ns"
    subject_id = "[s]"
    relation_id = "[r]"
    object_id = "[o]"
    et_id = "[e]"

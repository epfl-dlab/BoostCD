...
def run_process_predictions(cfg: DictConfig) -> Dict[str, Dict[str, Union[str, float, List[float]]]]:
    ...
    if cfg.get("linearization_class_id", None):
        ...
        log.info("Linearization class ID not specified. Using the default one `fully_expanded`")
    ...
    if cfg.wandb_run_path:
        ...
        cfg.output_dataset.data_dir = cfg.data_path
...
@hydra.main(version_base="1.2", config_path=ROOT_DIR + "/configs/boostie", config_name="process_predictions_root")
